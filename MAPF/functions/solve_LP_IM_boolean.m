...
try
    ...
    if flag_ILP
        clear Aeq2 beq2 Aineq2 bineq2 upper
        ...
        Aineq2 = zeros(num_intermediate*nplaces+2*np+nc,num_intermediate*(nplaces+ntrans)+np);
        bineq2 = [];
        ...
        %add constraints regarding the final marking and x*
        Aineq2(num_intermediate*nplaces+1:num_intermediate*nplaces+np,(num_intermediate-1)*(nplaces+ntrans)+1:(num_intermediate-1)*(nplaces+ntrans)+nplaces) = -V; %x-V^T*x <= 0
        Aineq2(num_intermediate*nplaces+1:num_intermediate*nplaces+np,num_intermediate*(nplaces+ntrans)+1:num_intermediate*(nplaces+ntrans)+np) = eye(np);
        bineq2(num_intermediate*nplaces+1:num_intermediate*nplaces+np) = zeros(np,1);

        Aineq2(num_intermediate*nplaces+np+1:num_intermediate*nplaces+np+np,(num_intermediate-1)*(nplaces+ntrans)+1:(num_intermediate-1)*(nplaces+ntrans)+nplaces) = V; %V^T*x-M*x <= 0
        Aineq2(num_intermediate*nplaces+np+1:num_intermediate*nplaces+np+np,num_intermediate*(nplaces+ntrans)+1:num_intermediate*(nplaces+ntrans)+np) = -M*eye(np);
        bineq2(num_intermediate*nplaces+np+1:num_intermediate*nplaces+np+np) =  zeros(np,1);
        ...
        upper = [inf*ones(1,num_intermediate*(nplaces+ntrans)) ones(1,np)];
